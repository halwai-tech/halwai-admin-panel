...
const HalwaiList = () => {
  const [halwaiList, setHalwaiList] = useState([]);
  ...
  const fetchUsers = async (pageNumber: number, limit: number) => {
    try {
      setLoading(true);
      const response = await userController.getUsersByRoles("halwai",pageNumber+1,limit);
      const responseData = response?.data;

      setHalwaiList(responseData?.data || []);
